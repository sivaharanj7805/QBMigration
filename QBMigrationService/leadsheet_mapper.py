...
class LeadSheetMapper:
        ...
        with self._detection_lock:
            ...
            country = (
                (
                    ...
                    or company_data.get("Country")
                    or company_data.get("companyCountry")
                    or ""
                )
                .strip()
                .upper()
            ...
            if country in ifrs_countries or currency in [
                "GBP",
                "EUR",
                "AUD",
                "NZD",
                ...
                logger.info(
                    f"Detected IFRS from company data (Country: {country}, Currency: {currency})"
                )
            ...
            # Default: US GAAP
            logger.info(
                f"Defaulting to US_GAAP (Country: {country or 'unknown'}, Currency: {currency or 'USD'})"
            )
            self.detected_standard = "US_GAAP"
            return self.detected_standard

    def get_lead_sheet_codes(
        self, accounting_standard: Optional[AccountingStandard] = None
    ) -> Dict[str, str]:
        """
    ...
    def get_type_code(self, account_type: str) -> str:
        ...
        type_map = {
            ...
            "Sales": "R",
            "Service Income": "R",
            "Agricultural Sales": "R",
            "Manufacturing Sales": "R",
            "Contract Revenue": "R",
            # COGS
        ...
        if code is None:
            logger.warning(
                f"Unknown account type for type code mapping: '{account_type}' - defaulting to 'O' (Other)"
            )
            return "O"
        return code
