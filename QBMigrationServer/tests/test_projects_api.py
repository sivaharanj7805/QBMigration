...
class TestListProjects:
    ...
    def test_list_projects_has_required_fields(self, authenticated_client, db_session, test_user, sample_project):
        ...
        data = response.get_json()
        project = data['projects'][0]
        for field in ['id', 'name', 'client_name', 'status', 'session_id',
                       'tier_type', 'tier_name', 'transaction_limit',
                       'transactions_used', 'created_at']:
...
class TestCreateProject:
    ...
    def test_create_project_consumes_credit(self, authenticated_client, db_session, test_user, paid_credit):
        ...
        authenticated_client.post('/api/projects', json={
            'name': 'Credit Test',
            'client_name': 'Client'
        })
...
class TestUpdateProject:
    ...
    def test_update_project_name(self, authenticated_client, db_session, test_user, sample_project):
        ...
        response = authenticated_client.put(f'/api/projects/{sample_project.id}', json={
            'name': 'Updated Name'
        })
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_update_project_not_found(self, authenticated_client, db_session, test_user):
        ...
        response = authenticated_client.put('/api/projects/99999', json={
            'name': 'Nope'
        })
        assert response.status_code == 404
    ...
    def test_update_project_other_user(self, authenticated_client, db_session, test_user):
        ...
        response = authenticated_client.put(f'/api/projects/{other_project.id}', json={
            'name': 'Hacked Name'
        })
        assert response.status_code == 404
...
class TestDeleteProject:
    ...
    def test_delete_project_blocked_by_active_migration(self, authenticated_client, db_session, test_user, sample_project):
    ...
    def test_delete_project_allowed_with_completed_migration(self, authenticated_client, db_session, test_user, sample_project):
    ...
    def test_delete_project_blocked_by_uploading_migration(self, authenticated_client, db_session, test_user, sample_project):
    ...
    def test_delete_project_blocked_by_migrating_migration(self, authenticated_client, db_session, test_user, sample_project):
