...
def list_vault():
    ...
    try:
        ...
        archived = Migration.query.filter(
        ...
        for m in archived:
            ...
            companies.append({
                'id': m.migration_id,
                'companyName': m.company_name or m.qb_file_name or 'Unknown Company',
                'archiveDate': m.completed_at.isoformat() if m.completed_at else m.created_at.isoformat(),
                'recordCount': record_count,
                'storageClass': 'S3 Glacier' if m.cleanup_completed else 'S3 Standard',
                'status': 'archived' if m.status == 'completed' else 'cleaned',
                'retentionYears': 7,  # Default forensic retention
                'lastAccessed': m.completed_at.isoformat() if m.completed_at else m.created_at.isoformat(),
            })
...
def restore_vault_item(migration_id):
    ...
    try:
        ...
        logger.info(f"Vault restore requested: migration {migration_id} by user {user_id}")
        ...
        logger.exception(f"Failed to restore vault item {migration_id} for user {user_id}: {e}")
        db.session.rollback()
