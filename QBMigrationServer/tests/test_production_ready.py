...
requires_server = pytest.mark.skipif(
    not _server_is_running(), reason=f"API server not running at {API_BASE_URL}"
)
...
class TestProductionAuthentication:
    ...
    def test_token_validation_real_api(self):
        ...
        response = requests.get(
            ...
            headers={
                "Authorization": f"Bearer {TestProductionAuthentication.auth_token}"
            },
            timeout=30,
        )
...
class TestProductionLicenseAPI:
    ...
    def test_license_validation_requires_key(self):
        ...
        response = requests.post(
            f"{API_BASE_URL}/api/license/validate", json={}, timeout=30
        )
...
class TestProductionAPIEndpoints:
    ...
    @pytest.mark.parametrize(
        "endpoint,method,expected_min,expected_max",
        [
    ...
    def test_endpoint_responds(self, endpoint, method, expected_min, expected_max):
        ...
        assert (
            expected_min <= response.status_code <= expected_max
...
class TestProductionSecurity:
    ...
    def test_sql_injection_prevented(self):
        ...
        for malicious in malicious_inputs:
            response = requests.post(
                f"{API_BASE_URL}/api/auth/login",
                json={"email": malicious, "password": malicious},
                timeout=30,
            )
...
class TestProductionConfiguration:
    ...
    def test_data_retention_code_default(self):
        ...
        assert (
            "'2555'" in content or '"2555"' in content
    ...
    def test_aws_region_code_default(self):
        ...
        assert (
            "'ca-central-1'" in content or '"ca-central-1"' in content
