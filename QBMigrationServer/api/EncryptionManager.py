...
class EncryptionManager:
    ...
    def _load_or_generate_keys(self) -> Tuple[any, str]:
        ...
        if os.path.exists(self.private_key_path):
            ...
            try:
                ...
                logger.info(f"Loaded existing RSA-4096 key pair (version: {version}, created: {created_at})")
    ...
    def rotate_keys(self) -> Dict:
        ...
        if os.path.exists(self.private_key_path):
            ...
            if os.path.exists(self.public_key_path):
                ...
                shutil.copy2(self.public_key_path, os.path.join(self.archive_dir, pub_archive_name))

        # Generate new keys
        self.private_key, self.key_version = self._generate_new_keypair(is_rotation=True)
        self.public_key = self.private_key.public_key()
    ...
    def _prune_archived_keys(self):
        ...
        try:
            archived_files = sorted(
                [f for f in os.listdir(self.archive_dir) if f.startswith('rsa_private_')],
    ...
    def get_key_status(self) -> Dict:
        ...
        archived_count = 0
        try:
            archived_count = len([f for f in os.listdir(self.archive_dir) if f.startswith('rsa_private_')])
        except Exception:
            pass
        ...
        return {
            ...
            'rotation_due_in_days': max(0, KEY_ROTATION_DAYS - (age_days or 0)) if age_days else None,
            'algorithm': 'RSA-4096 OAEP-SHA256',
            'archived_key_count': archived_count,
            'max_archived_keys': MAX_ARCHIVED_KEYS
        }

    def decrypt_with_archived_key(self, encrypted_aes_key_b64: str, key_version: str) -> bytes:
        """
