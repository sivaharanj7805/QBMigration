...
class TestWebhooks:
    ...
    def _make_headers(self, webhook_secret, migration_id,
                      timestamp=None, webhook_id=None):
    ...
    @pytest.mark.parametrize('omit_header', [
        'X-Migration-Id',
        'X-Webhook-Signature',
        'X-Webhook-Timestamp',
        'X-Webhook-Id',
    ])
    def test_missing_single_header_returns_400(self, client, webhook_secret,
                                               webhook_migration, omit_header):
        """Omitting any single required header must return 400."""
        headers = self._make_headers(
            webhook_secret, webhook_migration.migration_id,
        )
    ...
    def test_expired_timestamp_returns_401(self, client, webhook_secret,
                                           webhook_migration):
        ...
        assert response.status_code == 401
        data = response.get_json()
        assert 'expired' in data.get('error', '').lower() or \
               'verification failed' in data.get('error', '').lower()

    # ===============================================================
    # 5. Future timestamp -> 401
    # ===============================================================

    def test_future_timestamp_returns_401(self, client, webhook_secret,
                                          webhook_migration):
        ...
        assert response.status_code == 401
        data = response.get_json()
        assert 'future' in data.get('error', '').lower() or \
               'verification failed' in data.get('error', '').lower()
    ...
    def test_migration_started_valid_signature(self, client, webhook_secret,
                                               webhook_migration, db_session):
        ...
        updated = db_session.query(Migration).filter_by(
    ...
    def test_migration_progress_valid_signature(self, client, webhook_secret,
                                                webhook_migration, db_session):
    ...
    def test_migration_progress_clamps_to_100(self, client, webhook_secret,
                                              webhook_migration, db_session):
    ...
    def test_migration_completed_valid_signature(self, client, webhook_secret,
                                                 webhook_migration, db_session):
    ...
    def test_migration_failed_valid_signature(self, client, webhook_secret,
                                              webhook_migration, db_session):
    ...
    def test_duplicate_webhook_id_is_idempotent(self, client, webhook_secret,
                                                webhook_migration, db_session):
        ...
        signature = make_signature(
            webhook_secret, webhook_migration.migration_id, timestamp,
        )
    ...
    def test_duplicate_webhook_id_idempotent_all_endpoints(
        self, client, webhook_secret, webhook_migration, db_session, endpoint,
    ):
    ...
    def test_migration_not_found_returns_404(self, client, webhook_secret,
                                             db_session, test_user):
        """Referencing a non-existent migration_id should return 404."""
        fake_migration_id = str(uuid.uuid4())
        headers = self._make_headers(
            webhook_secret, fake_migration_id,
        )
    ...
    def test_migration_not_found_all_endpoints(self, client, webhook_secret,
                                               db_session, test_user, endpoint):
    ...
    def test_non_json_content_type_returns_500(self, client, webhook_secret,
                                               webhook_migration):
    ...
    def test_empty_json_body_accepted(self, client, webhook_secret,
                                      webhook_migration, db_session):
    ...
    def test_timestamp_with_z_suffix_accepted(self, client, webhook_secret,
                                              webhook_migration):
    ...
    def test_timestamp_without_timezone_accepted(self, client, webhook_secret,
                                                 webhook_migration):
    ...
    def test_slightly_future_timestamp_within_tolerance(
        ...
        headers = self._make_headers(
            webhook_secret, webhook_migration.migration_id, timestamp=ts,
        )
    ...
    def test_progress_step_truncated_to_255(self, client, webhook_secret,
                                            webhook_migration, db_session):
    ...
    def test_webhook_secret_not_configured_returns_401(
        self, client, app, webhook_migration,
    ):
        ...
        try:
            ...
            assert response.status_code == 401
            data = response.get_json()
            assert 'not configured' in data['error'].lower() or \
                   'verification failed' in data['error'].lower()
        finally:
            # Always restore to avoid polluting other tests
            app.config['WEBHOOK_SECRET'] = original_secret

    def test_invalid_timestamp_format_returns_401(self, client, webhook_secret,
                                                  webhook_migration):
        """A non-ISO timestamp string should be rejected."""
        bad_ts = 'not-a-timestamp'
        headers = self._make_headers(
            webhook_secret, webhook_migration.migration_id, timestamp=bad_ts,
        )
