...
class TestAuthentication:
    ...
    def test_register_success(self, client, db_session):
        ...
        assert (
            "password" not in str(response.data).lower()
        ), "CRITICAL: Password leaked in response"
        ...
        ), "Cannot login with registered password"
    ...
    def test_register_weak_password(self, client):
        ...
        for weak_pwd in weak_passwords:
            response = client.post(
                "/api/auth/register",
                json={"email": f"test_{weak_pwd}@example.com", "password": weak_pwd},
            )
    ...
    def test_login_nonexistent_user(self, client):
        ...
        assert (
            "not found" not in data["error"].lower()
        ), "SECURITY: Error reveals user doesn't exist (enumeration vulnerability)"
        assert (
            "does not exist" not in data["error"].lower()
        ), "SECURITY: Error reveals user doesn't exist (enumeration vulnerability)"
    ...
    def test_account_lockout(self, client, test_user, db_session):
        ...
        for i in range(5):
            client.post(
                "/api/auth/login",
                json={"email": "test@example.com", "password": "WrongPassword1234"},
            )
    ...
    def test_password_history(self, client, test_user, db_session):
        ...
        assert (
            ...
            or "reuse" in str(exc_info.value).lower()
...
class TestUpload:
    ...
    def test_upload_authenticated(self, authenticated_client, test_user):
        ...
        response = authenticated_client.post(
            "/api/upload",
            json={"encrypted_data": fake_data, "company_name": "Test Company"},
        )
        ...
        if response.status_code == 500:
            ...
            assert (
                "aws" in error_lower
                or "s3" in error_lower
                or "storage" in error_lower
                or "upload" in error_lower
    ...
    def test_upload_unauthenticated(self, client):
        ...
        response = client.post("/api/upload", json={"encrypted_data": fake_data})
    ...
    def test_upload_invalid_format(self, authenticated_client, test_user):
        ...
        response = authenticated_client.post(
            "/api/upload", json={"encrypted_data": invalid_data}
        )

        assert (
            response.status_code == 400
        ), f"Invalid format accepted! Status: {response.status_code}"
        data = json.loads(response.data)
    ...
    def test_upload_empty_data(self, authenticated_client, test_user):
        ...
        """
        response = authenticated_client.post("/api/upload", json={"encrypted_data": ""})

        assert response.status_code == 400, "Empty data was accepted!"
        data = json.loads(response.data)
        assert (
            "required" in data["error"].lower() or "empty" in data["error"].lower()
    ...
    def test_upload_oversized_file(self, authenticated_client, test_user):
        ...
        response = authenticated_client.post(
            "/api/upload", json={"encrypted_data": huge_data}
        )
    ...
    def test_upload_sql_injection(self, authenticated_client, test_user):
        ...
        for malicious_name in malicious_names:
            response = authenticated_client.post(
                "/api/upload",
                json={"encrypted_data": fake_data, "company_name": malicious_name},
            )

            # Should either process safely or reject
            # Most importantly, should not cause SQL error
            assert (
                response.status_code != 500 or "sql" not in str(response.data).lower()
    ...
    def test_duplicate_detection(self, client, test_user, db_session):
        ...
        migration = Migration(
            user_id=test_user.id, file_hash="abc123def456", status="uploaded"
        )
...
class TestMigrationModel:
    ...
    def test_create_migration(self, db_session, test_user):
        ...
        """
        migration = Migration(
            user_id=test_user.id, company_name="Test Company", data_size_bytes=1000000
        )
        ...
        ), f"Wrong default status: {migration.status}"

        # VERIFY: Timestamps
        assert migration.created_at is not None, "created_at not set"
        assert migration.created_at <= datetime.now(
            timezone.utc
        ...
        found = (
            ...
            .filter_by(migration_id=migration.migration_id)
            .first()
        )
    ...
    def test_create_migration_invalid_user(self, db_session):
        ...
        migration = Migration(
            user_id=99999, company_name="Test Company"  # Non-existent user
        )
    ...
    def test_cost_estimation(self, db_session, test_user):
        ...
        """
        migration = Migration(
            user_id=test_user.id, data_size_bytes=50 * 1024 * 1024  # 50MB
        )
        db_session.add(migration)
        db_session.commit()
        ...
        if migration.cost_breakdown:
            breakdown = (
                json.loads(migration.cost_breakdown)
                if isinstance(migration.cost_breakdown, str)
                else migration.cost_breakdown
            )
            assert (
                "s3" in str(breakdown).lower() or "ec2" in str(breakdown).lower()
    ...
    def test_error_encryption(self, db_session, test_user):
        ...
        assert (
            migration.error_message_encrypted != error_msg
        ), "CRITICAL: Error stored in plaintext!"
        assert (
            "password" not in migration.error_message_encrypted.lower()
    ...
    def test_webhook_replay_prevention(self, db_session, test_user):
        ...
        assert not migration.is_webhook_processed(
            webhook_id
        ...
        assert migration.is_webhook_processed(
            webhook_id
...
class TestSecurity:
    ...
    def test_password_hashing_argon2(self, test_user):
        ...
        assert test_user.password_hash.startswith(
            "$argon2"
    ...
    def test_2fa_verify(self, test_user, db_session):
        ...
        assert not test_user.verify_2fa_token(
            backup_codes[0]
...
class TestRateLimiting:
    ...
    def test_login_rate_limit(self, client):
        ...
        for i in range(12):
            response = client.post(
                "/api/auth/login",
                json={"email": "ratelimit@example.com", "password": "Test123"},
            )
