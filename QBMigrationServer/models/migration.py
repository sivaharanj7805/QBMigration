...
class Migration(db.Model):
    ...
    __table_args__ = (
        ...
        db.Index(
            "idx_migration_user_status_created", "user_id", "status", "created_at"
        ),
    ...
    )  # References Project.session_id (not FK for flexibility)
    ...
    aws_region = db.Column(
        db.String(50), default="ca-central-1"
    ...
    estimated_cost_usd = db.Column(
        db.Numeric(14, 6)
    )  # Increased from (12, 6) - prevents overflow
    actual_cost_usd = db.Column(
        db.Numeric(14, 6)
    )  # Increased from (12, 6) - handles large migrations
    cost_breakdown = db.Column(db.Text)  # JSON: {ec2: X, s3: Y, data_transfer: Z}

    # Forensic Data (Stored as JSON Text)
    trial_balance_data = db.Column(
        db.Text
    )  # JSON: Source/Dest balances, variance, hash
        ...
        try:
            ...
            if not key:
                ...
                if is_production:
                    ...
                    self.error_message_encrypted = (
                        "[REDACTED - encryption key not configured]"
                    )
                    return
    ...
    def find_duplicate(cls, user_id, file_hash):
        ...
        return (
            cls.query.filter_by(user_id=user_id, file_hash=file_hash)
            .filter(cls.status.in_(["uploaded", "processing", "completed"]))
            .first()
        )
    ...
    def estimate_cost(self):
        ...
        s3_cost = (
            size_gb * s3_cost_per_gb * (1 / 30)
        )  # Cost for 1 day out of 30-day month
    ...
    def set_actual_cost(self, cost_dict):
        ...
        self.cost_breakdown = json.dumps({k: round(v, 4) for k, v in cost_dict.items()})
    ...
    def mark_as_completed(self, results=None):
        ...
        if results is None:
            logger.error(
                "mark_as_completed called without results - trial balance verification required"
            )
            return False
        ...
        if self.status == "processing":
            ...
            if results and "trial_balance" in results:
                ...
                if (
                    not is_balanced or discrepancy > BALANCE_TOLERANCE
    ...
    def mark_webhook_processed(self, webhook_id):
        ...
        try:
            ...
            logger.warning(
                f"Webhook {webhook_id} - migration {self.id} is locked by another transaction: {lock_error}"
            )
            db.session.rollback()
            return False
        ...
        try:
            processed = (
                json.loads(self.webhook_processed_ids)
                if self.webhook_processed_ids
                else []
            )
        except (json.JSONDecodeError, TypeError) as e:
    ...
    def to_dict(self, include_sensitive=False):
        ...
        data = {
            ...
            "caseware_bundle_ready": (
                self.caseware_bundle_ready if self.destination == "caseware" else None
            ),
            ...
            "error_message": (
                self.get_error_message() if self.status == "failed" else None
            ),
            ...
            data.update(
                {
                    ...
                    "data_size_mb": (
                        round(self.data_size_bytes / (1024**2), 2)
            ...
            try:
                ...
                metadata = {
                    ...
                    "original_account_count": (
                        len(data.get("accounts", [])) if "accounts" in data else None
                    ),
        ...
        if self.live_status_data:
            try:
                ...
                metadata = {
                    "stripped_at": datetime.now(timezone.utc).isoformat(),
                    "final_phase": (
                        data.get("current_phase") if "current_phase" in data else None
                    ),
                    "phase_count": (
                        len(data.get("phases", [])) if "phases" in data else None
                    ),
                    "completion_status": (
                        data.get("status") if "status" in data else None
                    ),
