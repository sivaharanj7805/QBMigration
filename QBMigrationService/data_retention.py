        ...
        for file_path in job_data["file_paths"]:
            if os.path.exists(file_path):
                try:
                    ...
                    logger.log_deletion(
                        job_data["migration_id"], file_path, method="secure_7pass"
                    )
                    deleted_count += 1
...
class DataRetentionManager:
    ...
    def _load_jobs(self):
        ...
        if os.path.exists(self.jobs_file):
            try:
                ...
                logger.info(
                    f"✓ Resumed {len(self.deletion_jobs)} scheduled deletion jobs"
                )
            ...
            with open(self.jobs_file, "w") as f:
                ...
                for job in self.deletion_jobs:
                    ...
                    if isinstance(job_copy["deletion_time"], datetime):
                        job_copy["deletion_time"] = job_copy[
                            "deletion_time"
                        ].isoformat()
    ...
    def schedule_deletion(self, migration_id, file_paths, delay_hours=None):
        ...
        if CELERY_AVAILABLE:
            ...
            celery_execute_deletion.apply_async(
                args=[job], eta=deletion_time, task_id=f"deletion_{migration_id}"
            )

            logger.info(
                f"✓ Scheduled deletion via Celery in {delay_hours} hour(s) for {len(job['file_paths'])} file(s)"
            )
        else:
            ...
            logger.info(
                f"✓ Scheduled deletion via threading in {delay_hours} hour(s) for {len(job['file_paths'])} file(s)"
            )
    ...
    def _start_deletion_thread(self, job, delay_seconds):
        ...
        thread = threading.Thread(
            target=self._delete_after_delay, args=(job, delay_seconds)
        )
        thread.daemon = True
        thread.start()
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    self.logger.log_deletion(
                        job["migration_id"], file_path, method="secure_7pass"
                    )
    ...
    def cancel_deletion(self, migration_id):
        ...
        if cancelled_count > 0:
            logger.info(
                f"✓ Cancelled {cancelled_count} deletion job(s) for migration {migration_id}"
            )

            self.logger.log_security_event(
                "DELETION_CANCELLED",
                {"migration_id": migration_id, "job_count": cancelled_count},
            )
    ...
    def cleanup_completed_jobs(self, days_old=7):
        ...
        self.deletion_jobs = [
            ...
            or datetime.fromisoformat(
                job.get("completed_at", datetime.now().isoformat())
            )
            > cutoff
        ]
