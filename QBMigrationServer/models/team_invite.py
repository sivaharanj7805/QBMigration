...
class TeamInvite(db.Model):
    ...
    owner = db.relationship(
        "User", foreign_keys=[owner_user_id], backref="sent_invites"
    )
    accepted_user = db.relationship(
        "User", foreign_keys=[accepted_user_id], backref="received_invites"
    )

    @classmethod
    def create_invite(
        cls, owner_user_id, email, role="member", expiry_days=7, auto_commit=True
    ):
        """
    ...
    def get_pending_for_owner(cls, owner_user_id):
        ...
        return (
            cls.query.filter_by(owner_user_id=owner_user_id, status="pending")
    ...
    def get_team_members(cls, owner_user_id):
        ...
        accepted_invites = cls.query.filter_by(
            owner_user_id=owner_user_id, status="accepted"
        ).all()
    ...
    def cleanup_expired(cls):
        ...
        expired_count = cls.query.filter(
            cls.status == "pending", cls.expires_at < now
