...
class TestMasterEndToEnd:
    ...
    def test_complete_migration_pipeline(self, tmp_migration_dir, sample_qbd_data):
        ...
        assert all(
            len(h) == 64 for h in source_hashes.values()
        ...
        print(
            f"   ✓ Destination: s3://{upload_metadata['bucket']}/{upload_metadata['key']}"
        )
        ...
        with patch.object(client, "_process_single_batch") as mock_batch:
            ...
            for entity_type, entities in result["entities"].items():
                ...
                batch_result = client.batch_create_parallel(
                    entities=entities, entity_type=entity_type
                )
        ...
        ), "At least Account and Customer should be transformed"
