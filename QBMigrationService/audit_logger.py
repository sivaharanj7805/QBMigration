...
class AuditLogger:
    ...
    def _sanitize_string(self, text: str) -> str:
        ...
        text = re.sub(
            r"\b\d{4}[\s\-]?\d{4}[\s\-]?\d{4}[\s\-]?\d{4}\b", "[CC-REDACTED]", text
        )
        ...
        text = re.sub(r"\b([A-Z][a-z]+)\s+([A-Z][a-z]+)\b", r"\1 [NAME-REDACTED]", text)
    ...
    def log_data_operation(
        self, operation: str, entity_type: str, entity_count: int, migration_id: str
    ):
        """Log data operation"""
        details = {
            "operation": operation,
    ...
    def log_encryption(
        self, migration_id: str, action: str, file_size: Optional[int] = None
    ):
    ...
    def log_deletion(
        self, migration_id: str, file_path: str, method: str = "secure_7pass"
    ):
        """
        Log file deletion
    ...
    def log_migration_start(
        self, migration_id: str, company_name: str, data_counts: Dict[str, int]
    ):
        """Log migration start"""
        details = {
            "migration_id": migration_id,
    ...
    def log_migration_complete(
        self, migration_id: str, success: bool, duration_seconds: float
    ):
    ...
    def log_api_error(
        self, migration_id: str, endpoint: str, status_code: int, error_summary: str
    ):
        """
        Log API errors
