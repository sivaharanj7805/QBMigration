...
class RegionalEnforcement:
    ...
    def verify_region(region: str) -> Tuple[bool, str]:
        ...
        if region != REQUIRED_REGION:
            return (
                False,
                f"Region '{region}' not allowed. Must use '{REQUIRED_REGION}' (Montreal) for Canadian data residency.",
            )
        return True, None
    ...
    def verify_s3_bucket_location(s3_client, bucket_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if location != REQUIRED_REGION:
                return (
                    False,
                    f"S3 bucket '{bucket_name}' is in '{location}', must be in '{REQUIRED_REGION}'",
                )
            ...
            for reservation in response["Reservations"]:
                for instance in reservation["Instances"]:
                    ...
                    if az not in ALLOWED_AVAILABILITY_ZONES:
                        return (
                            False,
                            f"Instance '{instance_id}' in AZ '{az}', must be in {ALLOWED_AVAILABILITY_ZONES}",
                        )
...
class S3ObjectLocking:
    ...
    def enable_object_lock_on_bucket(self, bucket_name: str = None) -> dict:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "ObjectLockConfigurationNotFoundError":
                logger.warning(
                    f"Object Lock not enabled on bucket '{bucket}'. "
    ...
    def set_default_retention(
        self, bucket_name: str, mode: str = "GOVERNANCE", days: int = 2555
    ) -> dict:
        """
        ...
        try:
            ...
            logger.info(
                f"Set {mode} retention of {days} days on bucket '{bucket_name}'"
            )
    ...
    def put_object_with_lock(
        ...
        try:
            ...
            logger.info(
                f"Uploaded WORM object '{key}' with {retention_mode} retention until {retain_until}"
            )
    ...
    def store_forensic_manifest(
        self, migration_id: str, manifest_data: dict, retain_years: int = 7
    ) -> dict:
        """
    ...
    def store_forensic_log(
        self, migration_id: str, log_type: str, log_data: str, retain_years: int = 7
    ) -> dict:
        """
...
class CustomerManagedKeys:
    ...
    def validate_customer_key(self, key_id: str) -> Tuple[bool, dict]:
        ...
        try:
            ...
            if REQUIRED_REGION not in key_arn:
                return False, {
                    "error": f"Key must be in {REQUIRED_REGION} for Canadian data residency"
                }

            # Verify key is enabled
            if key_metadata["KeyState"] != "Enabled":
                return False, {
                    "error": f"Key is not enabled (state: {key_metadata['KeyState']})"
                }
            ...
            if error_code == "NotFoundException":
                ...
                return False, {
                    "error": "Access denied - ensure ForensicBridge has kms:DescribeKey permission"
                }
    ...
    def configure_bucket_cmk(self, bucket_name: str, customer_key_arn: str) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Configured bucket '{bucket_name}' with CMK '{customer_key_arn}'"
            )
    ...
    def upload_with_cmk(
        self, bucket_name: str, key: str, body: bytes, customer_key_arn: str
    ) -> dict:
        """
    ...
    def generate_cmk_grant(
        self, key_id: str, grantee_principal: str, operations: List[str] = None
    ) -> dict:
        """
...
class MultiAZDeployment:
    ...
    def select_optimal_zone(self, subnet_id: str = None) -> str:
        ...
        try:
            ...
            logger.info(
                f"Selected optimal zone: {optimal_zone} (instance counts: {zone_counts})"
            )
            return optimal_zone
    ...
    def launch_in_zone(self, zone: str, launch_params: dict) -> str:
        ...
        if zone not in ALLOWED_AVAILABILITY_ZONES:
            raise ValueError(
                f"Zone '{zone}' not in allowed zones: {ALLOWED_AVAILABILITY_ZONES}"
            )
