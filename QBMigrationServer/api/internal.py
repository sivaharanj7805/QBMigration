...
def require_internal_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        expected_key = os.environ.get("INTERNAL_API_KEY") or current_app.config.get(
            "INTERNAL_API_KEY"
        )

        if not expected_key:
            logger.error(
                "INTERNAL_API_KEY not configured - rejecting all internal API requests"
            )
            return (
                jsonify({"success": False, "error": "Internal API not configured"}),
                500,
            )
        ...
        if not provided_key:
            logger.warning(
                f"Internal API call without API key from {request.remote_addr}"
            )
...
def trigger_processing():
    ...
    logger.info(
        f"Processing triggered for session {session_id}, bucket: {s3_bucket}, key: {s3_key}"
    )
    ...
    try:
        ...
        if not re.match(uuid_pattern, session_id):
            logger.warning(
                f"Invalid session_id format from internal API: {session_id[:50]}"
            )
            return (
                jsonify({"success": False, "error": "Invalid session_id format"}),
                400,
            )
        ...
        if not migration:
            ...
            migration = Migration.query.filter(Migration.s3_key == s3_key).first()
        ...
        if migration.status not in ["pending", "uploading"]:
            logger.warning(
                f"Migration {session_id} in unexpected state {migration.status} for trigger"
            )
            return (
                jsonify(
                    {
        ...
        logger.info(
            f"Migration {migration.migration_id} updated, status: {migration.status}"
        )
