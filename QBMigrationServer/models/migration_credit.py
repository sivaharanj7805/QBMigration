...
class MigrationCredit(db.Model):
    ...
    transaction_limit = db.Column(db.Integer, nullable=False)  # Max transactions allowed
    ...
    # Usage tracking
    status = db.Column(db.String(50), default='pending')  # pending (unpaid), available, used, expired
    ...
    # Relationship
    user = db.relationship('User', backref=db.backref('migration_credits', lazy='dynamic'))
    ...
    def get_credits_summary(cls, user_id):
        ...
        """
        credits = cls.query.filter_by(user_id=user_id).filter(
            cls.payment_status == 'paid'  # Only count paid credits
        ).all()
    ...
    def to_dict(self):
        ...
        return {
            ...
            'tier_name': self.TIER_CONFIG.get(self.tier_type, {}).get('name', self.tier_type),
