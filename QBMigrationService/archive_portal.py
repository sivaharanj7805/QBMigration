...
def get_archive(archive_id):
    ...
    if not validate_archive_id(archive_id):
        logger.warning(
            f"Invalid archive_id attempted: {archive_id[:50] if archive_id else 'None'}"
        )
    ...
    log_access(
        archive_id, request.headers.get("X-User-Id", "anonymous"), "VIEW_ARCHIVE"
    )
...
def search_transactions(archive_id):
    ...
    if not validate_archive_id(archive_id):
        logger.warning(
            f"Invalid archive_id in transactions search: {archive_id[:50] if archive_id else 'None'}"
        )
        return jsonify({"error": "Invalid archive ID format"}), 400
    ...
    if search:
        results = [
            t
            for t in results
            if search in (t.get("Memo") or "").lower()
            or search in (t.get("EntityName") or "").lower()
            or search in (t.get("Number") or "").lower()
        ]
    ...
    log_access(
        ...
        f"SEARCH_TRANSACTIONS (q={search})",
    )
...
def get_transaction(archive_id, txn_id):
    ...
    if not validate_archive_id(archive_id):
        logger.warning(
            f"Invalid archive_id in transaction get: {archive_id[:50] if archive_id else 'None'}"
        )
        return jsonify({"error": "Invalid archive ID format"}), 400
...
def get_audit_log():
    ...
    limit = request.args.get("limit", 100, type=int)
    return jsonify({"total": len(entries), "entries": entries[:limit]})
