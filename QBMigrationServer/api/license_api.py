...
def get_license_secret():
    ...
    if (
        ...
        logger.warning(
            "LICENSE_SECRET_KEY not set - using SECRET_KEY fallback (non-production mode)"
        )
...
def generate_license_token(license_obj, hardware_fingerprint):
    ...
    payload = {
        ...
        + timedelta(hours=24),  # Token valid for 24 hours
    }
...
def validate_license():
    ...
    try:
        ...
        if not license_obj:
            logger.warning(
                f"License validation failed: key not found - {license_key[:15]}..."
            )
        ...
        if not is_valid:
            ...
            db.session.commit()
            logger.warning(
                f"License validation failed: {error} - License ID {license_obj.id}"
            )
        ...
        return jsonify(
            {
                "valid": True,
                "tier": license_obj.tier,
                "tier_name": LICENSE_TIERS.get(license_obj.tier, {}).get(
                    "name", license_obj.tier
                ),
                "migrations_remaining": license_obj.get_migrations_remaining(),
...
def activate_license():
    ...
    try:
        ...
        if not license_obj:
            logger.warning(
                f"License activation failed: key not found - {license_key[:15]}..."
            )
        ...
        if (
            ...
            and datetime.now(timezone.utc) > license_obj.expires_at
        ):
        ...
        if not success:
            ...
            db.session.commit()
            logger.warning(
                f"License activation failed: already activated elsewhere - ID {license_obj.id}"
            )
            return (
                jsonify(
                    {
        ...
        logger.info(
            f"License activated: ID {license_obj.id} on hardware {hardware_fingerprint[:20]}..."
        )
...
def get_usage():
    ...
    try:
        ...
        if not license_key or not hardware_fingerprint:
            return (
                jsonify({"error": "License key and hardware fingerprint required"}),
                400,
            )
...
def use_migration():
    ...
    try:
        ...
        if not license_key or not hardware_fingerprint:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "License key and hardware fingerprint required",
                    }
                ),
                400,
            )
        ...
        logger.info(
            f"Migration used: License ID {license_obj.id}, remaining: {license_obj.get_migrations_remaining()}"
        )
...
def create_license():
    ...
    try:
        ...
        logger.info(
            f"License created: {license_obj.license_key} ({tier}) by {hash_email(current_user.email)}"
        )
...
def deactivate_license():
    ...
    try:
        ...
        # FIX #53: Redact PII from logs
        logger.info(
            f"License deactivated: {license_key[:15]}... by {hash_email(current_user.email)}"
        )
...
def revoke_license():
    ...
    try:
        ...
        # FIX #53: Redact PII from logs
        logger.warning(
            f"License revoked: {license_key[:15]}... by {hash_email(current_user.email)}: {reason}"
        )

        return jsonify(
            {"success": True, "message": "License has been permanently revoked"}
        )
...
def get_license_history(license_id):
    ...
    try:
        ...
        activations = (
            LicenseActivation.query.filter_by(license_id=license_id)
