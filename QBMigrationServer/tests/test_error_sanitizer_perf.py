...
class TestCompiledPatterns:
    ...
    def test_compiled_patterns_are_regex_objects(self):
        ...
        for compiled_re, replacement in _COMPILED_SENSITIVE_PATTERNS:
            assert isinstance(compiled_re, re.Pattern), \
...
class TestSanitizationPerformance:
    ...
    SAMPLE_ERRORS = [
        ...
        # Token patterns
        Exception('Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkw.dozjgNryP4J3jVmNHl0w'),
        # Connection strings
    ...
    def test_single_sanitize_under_1ms(self):
        ...
        error = Exception('duplicate key value violates unique constraint "users_email_key"')
        ...
        assert avg_ms < 1.0, \
            f"Average sanitize time: {avg_ms:.3f}ms (expected < 1ms)"
    ...
    def test_compile_time_saved(self):
        ...
        assert precompiled_time <= onthefly_time * 1.2, \
...
class TestPatternCorrectness:
    ...
    def test_duplicate_key_error(self):
        result = self._match('duplicate key value violates unique constraint "users_email_key"')
...
class TestCreateErrorResponsePerformance:
    ...
    def test_response_creation_speed(self):
        ...
        errors = [
            ValueError("test"), KeyError("missing"), TypeError("wrong"),
        ]
