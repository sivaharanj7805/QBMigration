...
class EncryptionManager:
    ...
    if _ENV_SALT:
        ...
        DEFAULT_KDF_SALT = _hs.sha256(
            f"QBMigration-KDF-{_machine_id}".encode()
        ).digest()
    ...
    def decrypt_chunked(
        ...
        plaintext_bytes = EncryptionManager.decrypt_data(
            ciphertext=encrypted_data, key=key, iv=iv, tag=tag
        )
    ...
    @staticmethod
    def decrypt_from_json_with_verification(
        encrypted_json: Union[str, Dict],
    ) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            if len(tag) != 16:
                raise ValueError(
                    f"Invalid tag length: expected 16 bytes, got {len(tag)}"
                )
            if len(key) != 32:
                raise ValueError(
    ...
    def verify_data_integrity(
        plaintext: str, expected_hash: Optional[str], allow_legacy: bool = False
    ) -> bool:
        """
        ...
        if expected_hash is None:
            ...
            logger.info(
                f"   Current data hash: {current_hash[:16]}... (save for future reference)"
            )
    ...
    def decrypt_from_json(encrypted_json: Union[str, Dict]) -> str:
        ...
        plaintext, expected_hash = (
            EncryptionManager.decrypt_from_json_with_verification(encrypted_json)
        )
    ...
    def decrypt_string(encrypted: str) -> str:
        ...
        if encrypted.strip().startswith("{"):
            try:
                plaintext, _ = EncryptionManager.decrypt_from_json_with_verification(
                    encrypted
                )
                return plaintext
        ...
        try:
            ...
            logger.error(
                f"Decryption failed with unexpected error: {type(e).__name__}",
                exc_info=True,
            )
    ...
    @staticmethod
    def decrypt_file(
        input_path: str, output_path: str, verify_hash: bool = True
    ) -> bool:
        """
