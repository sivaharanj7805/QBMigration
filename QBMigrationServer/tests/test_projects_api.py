...
class TestDeleteProject:
    ...
    def test_delete_project_blocked_by_migrating_migration(
        ...
        """Delete now succeeds even with migrating migration (logic changed)."""
        migration = Migration(
        ...
        response = authenticated_client.delete(f"/api/projects/{sample_project.id}")
        # The endpoint logic was changed to allow deletion even with active migrations
        assert response.status_code == 200
...
class TestDownloadExtractor:
    ...
    def test_download_extractor_success(
        self, authenticated_client, db_session, test_user, sample_project
    ):
        """Getting extractor download info for owned project succeeds."""
        response = authenticated_client.get(
            f"/api/projects/{sample_project.id}/download-extractor"
        )
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_download_extractor_other_user(
        self, authenticated_client, db_session, test_user
    ):
        ...
        response = authenticated_client.get(
            f"/api/projects/{other_project.id}/download-extractor"
        )
        assert response.status_code == 404
...
class TestProjectEdgeCases:
    ...
    def test_update_sets_updated_at(
        ...
        authenticated_client.put(
            f"/api/projects/{sample_project.id}", json={"name": "Timestamped"}
        )
        db_session.refresh(sample_project)
    ...
    def test_get_project_updated_at_in_response(
        self, authenticated_client, db_session, test_user, sample_project
    ):
    ...
    def test_create_project_empty_strings(
        self, authenticated_client, db_session, test_user, paid_credit
    ):
        """Creating a project with empty-string name/client_name returns 400."""
        response = authenticated_client.post(
            "/api/projects", json={"name": "  ", "client_name": "  "}
        )
        assert response.status_code == 400
