...
class SearchFilter:
    ...
    entity_types: List[SearchEntityType] = field(
        default_factory=lambda: [SearchEntityType.ALL]
    )
...
class ArchiveSearchService:
    ...
    def search_by_ref_number(self, ref_number: str) -> List[SearchResult]:
        ...
        filters = SearchFilter(query=ref_number, fuzzy_match=False, max_results=50)
        response = self.search(filters)
        return response.results

    def search_by_amount(
        self, amount: Decimal, tolerance: Decimal = Decimal("0.01")
    ) -> List[SearchResult]:
        ...
        for inv in invoices:
            ...
            doc = {
                ...
                "amount": self._parse_decimal(
                    inv.get("Subtotal") or inv.get("subtotal")
                ),
    ...
    def _index_bills(self, bills: List[Dict], archive_id: str) -> int:
        ...
        for bill in bills:
            ...
            doc = {
                ...
                "amount": self._parse_decimal(
                    bill.get("AmountDue") or bill.get("amount_due")
                ),
    ...
    def _index_payments(self, payments: List[Dict], archive_id: str) -> int:
        ...
        for pmt in payments:
            ...
            doc = {
                ...
                "amount": self._parse_decimal(
                    pmt.get("TotalAmount") or pmt.get("total_amount")
                ),
    ...
    def _index_journal_entries(self, entries: List[Dict], archive_id: str) -> int:
        ...
        for je in entries:
            ...
            total = sum(
                self._parse_decimal(line.get("Amount") or line.get("amount"))
                or Decimal("0")
                for line in lines
            ...
            doc = {
                ...
                "searchable_text": self._build_searchable_text(
                    [
                        chk.get("RefNumber"),
                        chk.get("ref_number"),
                        chk.get("PayeeEntityRefFullName"),
                        chk.get("payee_name"),
                        chk.get("Memo"),
                        chk.get("memo"),
                    ]
    ...
    def _index_deposits(self, deposits: List[Dict], archive_id: str) -> int:
        ...
        for dep in deposits:
            ...
            doc = {
                ...
                "amount": self._parse_decimal(
                    dep.get("TotalAmount") or dep.get("total_amount")
                ),
    ...
    def _index_credit_memos(self, memos: List[Dict], archive_id: str) -> int:
        ...
        for cm in memos:
            ...
            doc = {
                ...
                "amount": self._parse_decimal(
                    cm.get("TotalAmount") or cm.get("total_amount")
                ),
                ...
                "searchable_text": self._build_searchable_text(
                    [
                        cm.get("RefNumber"),
                        cm.get("ref_number"),
                        cm.get("CustomerRefFullName"),
                        cm.get("customer_name"),
                    ]
                ),
            }
    ...
    def _index_sales_receipts(self, receipts: List[Dict], archive_id: str) -> int:
        ...
        for sr in receipts:
            ...
            doc = {
                ...
                "amount": self._parse_decimal(
                    sr.get("TotalAmount") or sr.get("total_amount")
                ),
    ...
    def _index_customers(self, customers: List[Dict], archive_id: str) -> int:
        ...
        for cust in customers:
            ...
            doc = {
                ...
                "date": None,
                "amount": self._parse_decimal(
                    cust.get("Balance") or cust.get("balance")
                ),
    ...
    def _index_vendors(self, vendors: List[Dict], archive_id: str) -> int:
        ...
        for vend in vendors:
            ...
            doc = {
                ...
                "date": None,
                "amount": self._parse_decimal(
                    vend.get("Balance") or vend.get("balance")
                ),
    ...
    def _index_items(self, items: List[Dict], archive_id: str) -> int:
        ...
        for item in items:
            ...
            doc = {
                ...
                "date": None,
                "amount": self._parse_decimal(
                    item.get("SalesPrice") or item.get("sales_price")
                ),
                ...
                "searchable_text": self._build_searchable_text(
                    [
                        ...
                        item.get("FullName"),
                        item.get("full_name"),
                        item.get("SalesDescription"),
                        item.get("sales_description"),
                    ]
                ),
            }
    ...
    def _index_accounts(self, accounts: List[Dict], archive_id: str) -> int:
        ...
        for acct in accounts:
            ...
            doc = {
                ...
                "date": None,
                "amount": self._parse_decimal(
                    acct.get("Balance") or acct.get("balance")
                ),
    ...
    def _find_candidates(self, query_terms: List[str], fuzzy: bool) -> set:
    ...
    def _score_match(
        self, record: Dict, query_terms: List[str], fuzzy: bool
    ) -> Tuple[float, str, str]:
