...
def list_reports():
    ...
    try:
        ...
        for m in migrations:
            ...
            # Audit Certificate
            cert_path = os.path.join(current_app.root_path, 'certificates', f'{m.migration_id}_audit_certificate.pdf')
                    ...
                    if verification_data.get('discrepancies'):
                        reports.append({
                            **base_report,
                            ...
                            'date': m.completed_at.strftime('%Y-%m-%d') if m.completed_at else 'N/A',
                            'status': 'ready',
...
def download_report(report_id):
    ...
    try:
        ...
        if report_type == 'certificate':
            report_path = os.path.join(current_app.root_path, 'certificates', f'{migration_id}_audit_certificate.pdf')
        else:
            reports_dir = get_report_storage_dir()
            report_path = os.path.join(reports_dir, f'{report_id}.pdf')
...
def get_discrepancies(migration_id):
    ...
    try:
        ...
        if hasattr(migration, 'verification_results') and migration.verification_results:
            try:
                ...
                if 'discrepancies' in verification_data:
                    ...
                    total_discrepancy = sum(abs(d.get('difference', 0)) for d in discrepancies)
                ...
                # Check trial balance for discrepancy
                    ...
                    if abs(diff) > 0.01:  # More than 1 cent difference
                        ...
                        discrepancies = [{
                            ...
                            'difference': diff,
                            'severity': 'critical' if abs(diff) > 100 else 'warning',
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to get discrepancy data'
        }), 500
...
def download_discrepancy_report(migration_id):
    ...
    try:
        ...
        logger.exception(f"Failed to download discrepancy report for {migration_id}: {str(e)}")
        return jsonify({
            'success': False,
            'error': 'Failed to download discrepancy report'
        }), 500
...
def _generate_variance_report(migration, output_path):
    ...
    try:
        ...
        summary_data = [
            ...
            ['Total Assets', f"${tb.get('source_total', 0):,.2f}", f"${tb.get('destination_total', 0):,.2f}", f"${abs(tb.get('source_total', 0) - tb.get('destination_total', 0)):,.2f}"],
            ['Status', '', '', 'Balanced' if tb.get('is_balanced', False) else 'Variance Detected']
        ]
...
def _generate_health_report(migration, output_path):
    ...
    try:
        ...
        story.append(Paragraph(f"<b>Company:</b> {migration.company_name}", styles['Normal']))
        story.append(Paragraph(f"<b>Migration ID:</b> {migration.migration_id}", styles['Normal']))
        story.append(Paragraph(f"<b>Status:</b> {migration.status.upper()}", styles['Normal']))
        story.append(Spacer(1, 20))
...
def _generate_discrepancy_report(migration, output_path):
    ...
    try:
        ...
        if discrepancies:
            story.append(Paragraph("<b>Identified Discrepancies:</b>", styles['Heading2']))
            ...
            story.append(Paragraph("No discrepancies detected. Migration balanced to the penny.", styles['Normal']))
