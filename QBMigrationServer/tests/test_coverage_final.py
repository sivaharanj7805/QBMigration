...
class TestReportGenerationHelpers:
    ...
    def test_generate_variance_report_importerror_fallback(
        self, app, migration_with_verification
    ):
    ...
    def test_generate_health_report_importerror_fallback(
        self, app, migration_with_verification
    ):
    ...
    def test_generate_discrepancy_report_importerror_fallback(
        self, app, migration_with_verification
    ):
    ...
    def test_generate_variance_report_with_reportlab(
        self, app, migration_with_verification
    ):
    ...
    def test_generate_health_report_with_reportlab(
        self, app, migration_with_verification
    ):
    ...
    def test_generate_discrepancy_report_with_reportlab(
        self, app, migration_with_verification
    ):
        ...
        output_path = os.path.join(
            app.root_path, "reports", "test_discrepancy_full.pdf"
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    def test_generate_discrepancy_report_no_verification_data(
        self, app, migration_no_verification
    ):
    ...
    def test_generate_discrepancy_report_trial_balance_variance(
    ...
    def test_generate_variance_report_no_verification(
        self, app, migration_no_verification
    ):
...
class TestReportGenerateEndpoint:
    ...
    def test_generate_variance_report(
        self, authenticated_client, completed_migration, app
    ):
    ...
    def test_generate_certificate_redirect(
        self, authenticated_client, completed_migration
    ):
...
class TestReportDownloadEndpoint:
    ...
    def test_invalid_migration_id_in_report(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/reports/notauuid_variance/download")
        assert response.status_code == 400
...
class TestDiscrepancyReportDownload:
    ...
    def test_download_discrepancy_not_found(
...
class TestWebhookFailedCleanupPaths:
    ...
    def test_failed_webhook_success(
        ...
        updated = (
            ...
            .filter_by(migration_id=processing_migration.migration_id)
            .first()
        )
...
class TestConfigValidation:
    ...
    def test_validate_aws_region_us_ami_in_canada(self, app):
        """US AMI in ca-central-1 region should raise ValueError (not just warn)."""
        from config import Config
        ...
        try:
            ...
            # This now raises a ValueError instead of just warning
            with pytest.raises(ValueError, match="DATA SOVEREIGNTY"):
                Config.validate_aws_region()
        finally:
            Config.AWS_REGION = original_region
    ...
    def test_warn_aws_credentials_production(self, app):
        """AWS credentials raise EnvironmentError in production."""
        from config import Config

        original_env = os.environ.get("FLASK_ENV")
        original_key = os.environ.get("AWS_ACCESS_KEY_ID")
        original_allow = os.environ.get("ALLOW_AWS_KEYS")
        try:
            os.environ["FLASK_ENV"] = "production"
            os.environ["AWS_ACCESS_KEY_ID"] = "test-key"
            if "ALLOW_AWS_KEYS" in os.environ:
                del os.environ["ALLOW_AWS_KEYS"]
            with pytest.raises(EnvironmentError, match="AWS access keys"):
            ...
            if original_allow:
                os.environ["ALLOW_AWS_KEYS"] = original_allow
...
class TestAuditCertificateDownload:
    ...
    def test_audit_certificate_not_found(
        self, authenticated_client, db_session, test_user
    ):
        """Non-existent migration should return 404."""
        fake_id = str(uuid.uuid4())
    ...
    def test_audit_certificate_generate_basic(
        self, authenticated_client, completed_migration, app
    ):
    ...
    def test_audit_certificate_already_exists(
        self, authenticated_client, completed_migration, app
    ):
...
class TestAuditCertificatePreview:
    ...
    def test_preview_completed_migration(
        self, authenticated_client, completed_migration
    ):
    ...
    def test_preview_pending_migration(
        self, authenticated_client, db_session, test_user
    ):
...
class TestCasewareStatus:
    ...
    def test_caseware_status_not_found(
        self, authenticated_client, db_session, test_user
    ):
        """Non-existent migration should return 404."""
        fake_id = str(uuid.uuid4())
...
class TestProjectsAPI:
    ...
    def test_create_project_no_credits(
        self, authenticated_client, db_session, test_user
    ):
    ...
    def test_download_extractor_not_found(
        self, authenticated_client, db_session, test_user
    ):
...
class TestListReports:
    ...
    def test_list_reports_with_completed_migration(
        ...
        migration.completed_at = datetime.now(timezone.utc)
        migration.verification_results = json.dumps(
            {"discrepancies": [{"account_name": "Cash", "difference": 1.00}]}
        )
        db_session.add(migration)
        db_session.commit()
...
class TestRecordCountAndDiscrepancies:
    ...
    def test_get_record_count_not_found(
        self, authenticated_client, db_session, test_user
    ):
        """Non-existent migration should return 404."""
        fake_id = str(uuid.uuid4())
    ...
    def test_get_discrepancies_not_found(
        self, authenticated_client, db_session, test_user
    ):
        """Non-existent migration should return 404."""
        fake_id = str(uuid.uuid4())
    ...
    def test_get_discrepancies_no_variance(
        self, authenticated_client, db_session, test_user
    ):
