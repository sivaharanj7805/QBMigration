...
class Project(db.Model):
    ...
    migration_credit_id = db.Column(
        db.Integer, db.ForeignKey("migration_credits.id"), nullable=True
    )
    tier_type = db.Column(
        db.String(50), nullable=False, default="starter"
    )  # starter, business, professional, enterprise, forensic
    transaction_limit = db.Column(
        db.Integer, nullable=False, default=5000
    )  # Max transactions allowed (-1 = unlimited)
    transactions_used = db.Column(
        db.Integer, default=0
    )  # Transactions extracted so far
    ...
    migration_credit = db.relationship(
        "MigrationCredit", backref=db.backref("project", uselist=False)
    )
