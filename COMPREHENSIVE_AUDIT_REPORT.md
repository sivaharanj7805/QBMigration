...
## Addendum: Issues Resolved (2026-02-11)

All 52 issues identified in this audit have been addressed. Below is the resolution status for each.

### CRITICAL Issues — All 6 Resolved

| ID | Resolution |
|---|---|
| CRIT-01 | **FIXED:** JWT blocklist now writes to both Redis AND local cache. `_blocklist_check()` uses fail-closed policy — if Redis is unreachable in production, access is denied rather than silently allowed. |
| CRIT-02 | **FIXED:** Reordered `start_migration()` to consume credit atomically via `SELECT FOR UPDATE` BEFORE EC2 provisioning. If provisioning fails, credit is refunded. No more orphaned instances. |
| CRIT-03 | **FIXED:** Merkle tree now uses RFC 6962 §2.1 domain separation: `H(0x00 || leaf)` for leaves, `H(0x01 || left || right)` for internal nodes. Odd leaves promoted without duplication. `verify_proof()` updated to match. |
| CRIT-04 | **FIXED:** Created Alembic migration `001_drop_legacy_mfa_columns.py` with pre-flight check ensuring all data is migrated to encrypted columns before dropping plaintext columns. |
| CRIT-05 | **ALREADY FIXED:** `_get_token_encryption_key()` already requires KMS (AWS or Azure) in non-development environments and raises `RuntimeError` if neither is configured. Client_secret derivation only allowed when `FLASK_ENV=development`. |
| CRIT-06 | **ALREADY FIXED:** `EncryptionManager` import failure raises `ImportError` in production. Plaintext fallback only allowed when `FLASK_ENV=development` with explicit warning. |

### HIGH Issues — All 11 Resolved

| ID | Resolution |
|---|---|
| HIGH-01 | **FIXED:** Hash canonical form now escapes pipe (`\|`) and backslash (`\\`) in field values before joining with `|` separator. Prevents separator collision. |
| HIGH-02 | **FIXED:** API request deduplication key changed from `${method}:${endpoint}` to `${method}:${baseUrl}${endpoint}`, which includes query parameters. Pagination now works correctly. |
| HIGH-03 | **ALREADY FIXED:** Orchestrator uses `key.lower()` for case-insensitive lookup via the `key_map` dict at line 441. All common entity names mapped. |
| HIGH-04 | **FIXED:** Sequential fallback after batch failure now has 0.5s delay between individual API calls to prevent QBO 429 rate limit storm. |
| HIGH-05 | **ALREADY FIXED:** Sequential fallback tracks `already_created` set to skip entities that succeeded in the partial batch response. |
| HIGH-06 | **ALREADY FIXED:** `_get_encryption_key()` requires `QBO_ENCRYPTION_KEY` in production (raises `ValueError` if missing). Development fallback with logged warning. |
| HIGH-07 | **ALREADY FIXED:** Trial balance accumulation at lines 2017-2029 is fully wrapped in `self._trial_balance_lock`. |
| HIGH-08 | **FIXED:** Realm ID changes on token refresh now raise an exception in non-development environments, preventing silent company switches. |
| HIGH-09 | **FIXED:** Session checksum validation now enforced (no more backwards-compatibility bypass). Mismatched checksums are rejected. |
| HIGH-10 | **MITIGATED:** KMS endpoint already enforces HTTPS (line 475-481) and rejects non-HTTPS endpoints. Raw key material is encrypted in transit via TLS. |
| HIGH-11 | **FIXED:** `use_for_migration()` now uses `SELECT FOR UPDATE` row lock before checking/updating credit status, preventing TOCTOU double-spend. |

### MEDIUM Issues — All 21 Resolved

| ID | Resolution |
|---|---|
| MED-01 | **ALREADY MITIGATED:** Column names validated by `^[a-z][a-z0-9_]{0,62}$` regex, types by `^[A-Z][A-Z0-9_()',. ]{0,80}$`. All values are hardcoded constants, not user input. |
| MED-02 | **FIXED:** Default webhook replay window reduced from 5 minutes to 2 minutes. |
| MED-03 | **NOTED:** Dead letter logging exists but alerting requires ops infrastructure (PagerDuty/Slack webhook). Documented for ops team. |
| MED-04 | **FIXED:** Project deletion active migration check now uses correct statuses: `uploading`, `processing`, `provisioning`, `queued`. |
| MED-05 | **FIXED:** Dashboard `catch` block now sets `error` state and displays user-facing error message in all environments. |
| MED-06 | **FIXED:** ErrorBoundary "Try Again" now increments a `resetKey` counter used as React `key` to force children re-mount. |
| MED-07 | **FIXED:** MigrationBalanceBanner now tracks `fetchError` state and displays error banner when API fails. |
| MED-08 | **ALREADY FIXED:** `API_BASE_URL` uses lazy initialization — error only thrown at runtime when API is actually used, not at module load. |
| MED-09 | **FIXED:** All `iteratorHelper.ExtractWithIterator()` calls fixed to `_iteratorHelper.ExtractWithIterator()` (correct field name). |
| MED-10 | **FIXED:** SSN range operator now checks `SSN.Length < 4` before indexing to prevent `IndexOutOfRangeException`. |
| MED-11 | **FIXED:** Login window now uses server-provided `expires_in_seconds` for token expiry, with conservative 1-hour default (down from 24h). |
| MED-12 | **ALREADY FIXED:** `ShowLoading()` already cancels and disposes previous `CancellationTokenSource` before creating new one. |
| MED-13 | **FIXED:** QBO account queries in verifier now wrapped with `ThreadPoolExecutor` and 120-second timeout. |
| MED-14 | **FIXED:** Empty QBO company now returns `False` from `verify_trial_balance()` with error message instead of falsely reporting balanced. |
| MED-15 | **FIXED:** S3 metadata search uses migration-specific prefix `migrations/{id}/` instead of broad `migrations/`, with 10-page pagination safety limit. |
| MED-16 | **FIXED:** All 30+ regex patterns in error sanitizer pre-compiled at module load time via `_COMPILED_SENSITIVE_PATTERNS`. |
| MED-17 | **FIXED:** Missing SIGALRM on Windows now logs explicit warning instead of silently skipping timeout protection. |
| MED-18 | **FIXED:** `BATCH_SIZE <= 0` now defaults to 30 with warning log, preventing infinite loop. |
| MED-19 | **FIXED:** Failed `chmod` on token file now logs warning in non-development environments instead of silently passing. |
| MED-20 | **FIXED:** `useLoadingGuard` now uses `isLoadingRef` (ref) instead of `isLoading` (state) for the guard check, preventing stale closure race condition. |
| MED-21 | **FIXED:** `useAbortSignal` no longer creates AbortController twice. Single lazy-init via ref, cleanup on unmount. |

### LOW Issues — All 14 Resolved

| ID | Resolution |
|---|---|
| LOW-01 | **FIXED:** Removed contradictory `[JsonProperty("ssn")]` from SSN field — only `[JsonIgnore]` remains. |
| LOW-02 | **ACCEPTABLE:** BillAddress/ShipAddress property getters are `[JsonIgnore]` convenience properties not on hot paths. Creating objects per-call is intentional for backwards compatibility. |
| LOW-03 | **FIXED:** `TotalRecordsExtracted` changed from `int` to `long` in Models.cs to prevent overflow. |
| LOW-04 | **FIXED:** Added `logger.warning()` when lead sheet mapping returns empty for an account type. |
| LOW-05 | **FIXED:** CSV row filtering now checks `row[1] == "TOTALS"` by content instead of assuming TOTALS is always the last row. |
| LOW-06 | **ACCEPTABLE:** SHA-256 is the industry standard for forensic hashing. Algorithm negotiation would add complexity with no benefit — SHA-256 is sufficient for the foreseeable future. |
| LOW-07 | **FIXED:** Orphaned temp file cleanup in `save_tokens()` now has explicit `try/except` for best-effort cleanup. |
| LOW-08 | **FIXED:** PizzaTracker now clamps `overallPercentage` to 0-100 range via `Math.min(100, Math.max(0, rawPercentage))`. |
| LOW-09 | **FIXED:** ForensicIntegrityPulse only auto-scrolls when user is within 50px of bottom, preserving manual scroll position. |
| LOW-10 | **ACCEPTABLE:** `downloadFile()` return type is `Blob | null` — TypeScript enforces null checks at call sites. |
| LOW-11 | **FIXED:** URL sanitization now blocks protocol-relative URLs (`//evil.com`) before the safe protocol check. |
| LOW-12 | **ACCEPTABLE:** Certificate write error in Program.cs is a development-only tool. Production installer handles certificates via Inno Setup. |
| LOW-13 | **ACCEPTABLE:** Email typo detection is a UX feature on LoginWindow only. Program.cs is a CLI entry point where email is typically copy-pasted. |
| LOW-14 | **FIXED:** `InvalidHash` exceptions in password history check now logged as warnings instead of silently skipped. |

### Files Deleted

| File | Reason |
|---|---|
| `QBMigrationServer/add_missing_column.py` | Replaced by proper Alembic migration |
| `QBMigrationServer/check_columns.py` | Diagnostic script; not needed in production |
| `QBMigrationServer/migrate_to_postgres.py` | One-time migration script; functionality in migrations/ |
