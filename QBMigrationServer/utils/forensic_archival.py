...
class ForensicArchivalService:
    ...
    def archive_audit_log(self, migration_id: str, audit_entries: List[dict]) -> dict:
        ...
        try:
            ...
            logger.info(f"Archived {len(audit_entries)} audit entries for {migration_id}")
        ...
        lifecycle_config = {
            'Rules': [
                ...
                },
                {
                    ...
                    'Filter': {
                        'Prefix': 'migrations/'  # Active migration data
                    },
                    'Status': 'Enabled',
                    'Expiration': {
        ...
        try:
            ...
            if 'ongoing-request="true"' in restore_status:
                ...
                return {
                    'key': key,
                    'storage_class': storage_class,
                    'restore_status': 'not_started' if storage_class in ['GLACIER', 'DEEP_ARCHIVE'] else 'not_required'
                }
                
        except ClientError as e:
            return {'error': str(e)}
    
    def list_archived_migrations(self, prefix: str = None, limit: int = 100) -> List[dict]:
        """
    ...
    def _sanitize_metadata(self, metadata: dict) -> dict:
        ...
        # Fields to preserve
        safe_fields = {
            'migration_id', 'user_id', 'company_id', 'company_name',
            'started_at', 'completed_at', 'status', 'error_code',
            'entity_counts', 'file_hashes', 'total_records',
            'qbo_company_id', 'migration_type', 'retry_count',
            'duration_seconds', 'aws_instance_id', 'aws_region'
        }
        
        # Fields to explicitly exclude
        sensitive_fields = {
            'access_token', 'refresh_token', 'password', 'secret',
            'email', 'phone', 'address', 'ssn', 'sin', 'tax_id',
