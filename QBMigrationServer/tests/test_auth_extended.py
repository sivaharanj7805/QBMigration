...
class TestGetTiers:
    ...
    def test_tiers_contains_expected_tiers(self, client, app):
        ...
        for expected_id in ['starter', 'business', 'professional', 'enterprise', 'forensic']:
...
class TestSelectTier:
    ...
    def test_select_tier_creates_migration_credit(self, authenticated_client, db_session, test_user):
        """Selecting a tier creates a MigrationCredit record."""
        authenticated_client.post('/api/auth/select-tier', json={
            'tier_id': 'business'
        })
    ...
    def test_select_tier_invalid_tier(self, authenticated_client, db_session, test_user):
    ...
    def test_select_tier_missing_body(self, authenticated_client, db_session, test_user):
        """Sending no JSON body returns error (auth layer intercepts bad request)."""
        response = authenticated_client.post('/api/auth/select-tier',
                                             data='',
    ...
    def test_select_tier_requires_auth(self, client, app):
        ...
        response = client.post('/api/auth/select-tier', json={
            'tier_id': 'starter'
        })
...
class TestForgotPassword:
    ...
    def test_forgot_password_prevents_enumeration(self, client, db_session, test_user):
        ...
        resp_existing = client.post('/api/auth/forgot-password', json={
            'email': 'test@example.com'
        })
        resp_missing = client.post('/api/auth/forgot-password', json={
...
class TestMFAVerify:
    ...
    def test_mfa_verify_no_body(self, authenticated_client, db_session, test_user):
        ...
        response = authenticated_client.post('/api/auth/mfa/verify',
                                             data='',
                                             content_type='application/json')
        # Empty body causes BadRequest during auth token parsing, resulting in 401
        assert response.status_code in (400, 401)

    def test_mfa_verify_invalid_code_format(self, authenticated_client, db_session, test_user):
    ...
    def test_mfa_verify_mfa_not_enabled(self, authenticated_client, db_session, test_user):
...
class TestTeam:
    ...
    def test_team_returns_pending_invites(self, authenticated_client, db_session, test_user):
...
class TestTeamInvite:
    ...
    def test_invite_requires_auth(self, client, app):
        ...
        response = client.post('/api/auth/team/invite', json={
    ...
    def test_invite_no_body(self, authenticated_client, db_session, test_user):
        ...
        response = authenticated_client.post('/api/auth/team/invite',
                                             data='',
                                             content_type='application/json')
        # Empty body causes BadRequest during auth token parsing, resulting in 401
        assert response.status_code in (400, 401)
...
class TestCheckCaptchaRequired:
    ...
    def test_check_captcha_returns_result(self, client, db_session, test_user):
        ...
        response = client.post('/api/auth/check-captcha-required', json={
            'email': 'test@example.com'
        })
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_check_captcha_no_body(self, client, db_session):
        ...
        response = client.post('/api/auth/check-captcha-required',
                               data='',
                               content_type='application/json')
        assert response.status_code == 400
...
class TestValidateSession:
    ...
    def test_validate_returns_user_info(self, authenticated_client, db_session, test_user):
    ...
    def test_validate_includes_profile_fields(self, authenticated_client, db_session, test_user):
...
class TestSyncCredits:
    ...
    def test_sync_credits_creates_missing_records(self, authenticated_client, db_session, test_user):
