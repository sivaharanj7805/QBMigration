...
class TestOrchestratorPluralToSingular:
    ...
    def test_plural_to_singular_mapping_exists(self):
        ...
        for entity_type in entity_order_types:
            assert entity_type in PLURAL_TO_SINGULAR, \
...
class TestTransformerOrchestratorIntegration:
    ...
    def test_full_entity_chain_transform(self, transformer):
        ...
        # 1. Accounts first
        accounts = [
            {'Name': 'Checking', 'AccountType': 'Bank', 'ListID': 'ACC-001', 'IsActive': True},
            {'Name': 'AR', 'AccountType': 'Accounts Receivable', 'ListID': 'ACC-002', 'IsActive': True},
            {'Name': 'Sales Income', 'AccountType': 'Income', 'ListID': 'ACC-003', 'IsActive': True},
        ]
        ...
        items = [
            {
                'Name': 'Widget', 'ItemType': 'Service', 'ListID': 'ITEM-001',
    ...
    def test_trial_balance_tracking(self, transformer):
        ...
        transformer.transform_entity('Accounts', {
            'Name': 'Cash', 'AccountType': 'Bank',
            'Balance': '5000.00', 'ListID': 'ACC-1', 'IsActive': True
        })

        # Credit-normal account
        transformer.transform_entity('Accounts', {
            'Name': 'AP', 'AccountType': 'Accounts Payable',
            'Balance': '5000.00', 'ListID': 'ACC-2', 'IsActive': True
        })
...
class TestFullIIFToQBOFlow:
    ...
    def test_full_iif_parse_to_qbo_structure(self):
        ...
        for cust in parsed['data']['customers']:
            transformer.transform_entity('Customers', {
                'Name': cust['NAME'],
                'ListID': f"CUST-{cust['NAME']}",
                'IsActive': True
            })
        ...
        for vend in parsed['data']['vendors']:
            transformer.transform_entity('Vendors', {
                'Name': vend['NAME'],
