        ...
        if data.get("success"):
            ...
            if migrations:
                ...
                for mig in migrations[:3]:
                    print_info(
                        f"  • {mig['migration_id']} - {mig['status']} - {mig.get('created_at', 'N/A')}"
                    )
            return True
        else:
...
def test_ec2_process_endpoint():
    ...
    response = SESSION.post(
        f"{BASE_URL}/api/migrations/{MIGRATION_ID}/process", json={"invalid": "data"}
    )
    ...
    if response.status_code == 200:
        ...
        if data.get("success"):
            ...
            status_response = SESSION.get(
                f"{BASE_URL}/api/migrations/{MIGRATION_ID}/status"
            )
