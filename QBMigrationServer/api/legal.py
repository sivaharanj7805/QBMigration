...
def dpa_api():
    ...
    return jsonify({
        ...
        'summary': {
            ...
            'sub_processors': [
                {'name': 'Amazon Web Services', 'location': 'USA/Canada', 'purpose': 'Cloud hosting'},
                {'name': 'Stripe Inc.', 'location': 'USA', 'purpose': 'Payment processing'},
                {'name': 'Intuit Inc.', 'location': 'USA', 'purpose': 'QuickBooks API access'}
            ],
...
def privacy_policy():
    """Privacy Policy page"""
    try:
        return render_template('legal/privacy.html', metadata=LEGAL_DOCUMENTS['privacy'])
    except Exception:
        return privacy_api()
...
def cookie_policy():
    """Cookie Policy page"""
    try:
        return render_template('legal/cookies.html', metadata=LEGAL_DOCUMENTS['cookies'])
    except Exception:
        return jsonify({
            'success': True,
            'document': 'cookies',
            ...
            'cookies_used': [
                {'name': 'session', 'purpose': 'Authentication', 'duration': '24 hours'},
                {'name': '_csrf_token', 'purpose': 'Security', 'duration': 'Session'},
                {'name': 'analytics_id', 'purpose': 'Usage analytics', 'duration': '1 year', 'optional': True}
            ]
        })
...
def security():
    ...
    try:
        return render_template('legal/security.html', metadata=LEGAL_DOCUMENTS['security'])
    except Exception:
        return security_api()
...
def data_processing():
    ...
    try:
        return render_template('legal/data-processing.html', metadata=LEGAL_DOCUMENTS['dpa'])
    except Exception:
        return dpa_api()
