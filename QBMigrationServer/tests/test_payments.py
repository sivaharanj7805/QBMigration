...
class TestGetTiers:
    ...
    def test_get_tiers_correct_structure(self, client, db_session):
        ...
        required_fields = ['id', 'name', 'price', 'price_cents', 'max_transactions', 'description', 'migrations']
        for tier in tiers:
            for field in required_fields:
                assert field in tier, f"Missing field '{field}' in tier {tier.get('id', 'unknown')}"
...
class TestGetCredits:
    ...
    def test_get_credits_with_paid_credit(self, authenticated_client, db_session, test_user):
    ...
    def test_get_credits_with_used_credit(self, authenticated_client, db_session, test_user):
    ...
    def test_get_credits_excludes_pending_unpaid(self, authenticated_client, db_session, test_user):
    ...
    def test_get_credits_multiple_types(self, authenticated_client, db_session, test_user):
        """Credits summary groups by tier type correctly."""
        for tier in ['starter', 'business', 'starter']:
            credit = MigrationCredit(
                user_id=test_user.id,
                tier_type=tier,
                transaction_limit=MigrationCredit.TIER_CONFIG[tier]['transaction_limit'],
...
class TestCreateCheckout:
    ...
    def test_create_checkout_stripe_not_configured(self, authenticated_client, db_session):
    ...
    def test_find_best_credit_smallest_suitable(self, db_session, test_user):
        ...
        for tier in ['professional', 'starter']:
            credit = MigrationCredit(
                ...
                transaction_limit=MigrationCredit.TIER_CONFIG[tier]['transaction_limit'],
                price_cents=MigrationCredit.TIER_CONFIG[tier]['price_cents'],
                stripe_checkout_session_id='cs_test_best_' + tier + '_' + str(uuid.uuid4()),
                stripe_payment_intent_id='pi_test_best_' + tier + '_' + str(uuid.uuid4()),
                payment_status='paid',
                status='available',
                paid_at=datetime.now(timezone.utc),
            )
