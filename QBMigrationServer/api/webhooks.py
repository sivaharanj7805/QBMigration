...
def verify_webhook_signature(migration_id, signature, timestamp):
    ...
    try:
        ...
        if age > max_age:
            logger.warning(f"Webhook expired: age={age.total_seconds()}s, max={max_age.total_seconds()}s")
            return False, f"Webhook expired (older than {max_age.total_seconds():.0f} seconds)"
...
def migration_started():
    ...
    try:
        ...
        if not is_valid:
            ...
            return jsonify({
                'success': False,
                'error': f'Webhook verification failed: {error}'
            }), 401
        ...
        return jsonify({
            'success': True,
            'message': 'Migration start acknowledged'
        }), 200
...
def migration_progress():
    ...
    try:
        ...
        logger.info(f"Migration {migration_id} progress: {progress_percent}% - {current_step}")
