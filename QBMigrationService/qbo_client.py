...
class PremiumQBOClient:
    ...
    def _get_plan_worker_limit(plan_name: str) -> int:
        ...
        """
        plan_limits = {"Advanced": 8, "Plus": 5, "Essentials": 3, "Simple Start": 2}
    ...
    def _init_database(self):
        ...
        with self.db_lock:
            ...
            conn = sqlite3.connect(
                str(self.db_path), check_same_thread=False, timeout=timeout
            )
            cursor = conn.cursor()
    ...
    def _make_request(  # noqa: C901
        ...
        if correlation_id:
            headers[getattr(config, "CORRELATION_ID_HEADER", "X-Correlation-ID")] = (
                correlation_id
            )
        ...
        try:
            if method == "POST":
                ...
                logger.warning(
                    "QBO uses POST for deletes, not DELETE. Use delete_entity() instead."
                )
                raise ValueError(
                    "QBO API does not support HTTP DELETE. Use POST with ?operation=delete via delete_entity()"
                )
            else:
            ...
            if getattr(config, "LOG_INTUIT_TID", True):
                logger.debug(
                    f"[{correlation_id}] Request {method} {endpoint} - Intuit TID: {intuit_tid}"
                )
            ...
            if response.status_code == 429:
                ...
                if retries < max_retries:
                    logger.warning(
                        f"[{correlation_id}] Rate limited (429). Waiting {wait_time}s... (TID: {intuit_tid})"
                    )
                    ...
                    raise Exception(
                        f"Max retries exceeded for rate limit (TID: {intuit_tid}, CID: {correlation_id})"
                    )
                ...
                if not _auth_refreshed and oauth_manager:
                    logger.warning(
                        f"[{correlation_id}] Token expired (401), refreshing..."
                    )
                    ...
                    raise Exception(
                        f"Authentication failed (TID: {intuit_tid}, CID: {correlation_id})"
                    )
            ...
            elif response.status_code in getattr(
                config, "RETRYABLE_STATUS_CODES", {429, 500, 502, 503, 504}
            ):
                if retries < max_retries:
                    wait_time = self._calculate_backoff(retries)
                    logger.warning(
                    ...
                    raise Exception(
                        f"Max retries exceeded for server error (TID: {intuit_tid}, CID: {correlation_id})"
                    )
            ...
            if isinstance(result, dict):
                for key in result:
                    if (
                        ...
                        and "Id" in result[key]
                        and "SyncToken" in result[key]
                    ):
            ...
            if retries < max_retries:
                wait_time = self._calculate_backoff(retries)
                logger.warning(
                    f"[{correlation_id}] Request timeout. Retry {retries+1}/{max_retries} in {wait_time}s..."
                )
                time.sleep(wait_time)
                return self._make_request(
                    method,
            ...
            if retries < max_retries:
                wait_time = self._calculate_backoff(retries)
                logger.warning(
                    f"[{correlation_id}] Request error. Retry {retries+1}/{max_retries} in {wait_time}s..."
                )
                time.sleep(wait_time)
                return self._make_request(
                    method,
    ...
    def delete_entity(
        ...
        delete_data = {"Id": qbo_id, "SyncToken": sync_token}

        try:
            self._make_request(
                "POST", endpoint, delete_data, oauth_manager=oauth_manager
            )
    ...
    def create_tax_service(
        ...
        response = self._make_request(
            "POST", "taxservice/taxcode", payload, oauth_manager=oauth_manager
        )
    ...
    def query_tax_agencies(self, oauth_manager: Optional[Any] = None) -> list:
        ...
        """
        response = self._make_request(
            "GET", "query?query=SELECT * FROM TaxAgency", oauth_manager=oauth_manager
        )
    ...
    def _process_single_batch(
        ...
        if not batch_data["BatchItemRequest"]:
            logger.info(
                f"All {len(batch)} entities in batch already migrated, skipping"
            )
        ...
        try:
            ...
            if not batch_responses:
                ...
                # Treat all as failed
                for entity in batch:
                    failed.append({"entity": entity, "error": "Empty API response"})
            ...
            for i, batch_item in enumerate(batch_responses):
                ...
                if batch_item.get(entity_type):
                    ...
                    qbd_id = (
                        original_entity.get("ListID")
                        or original_entity.get("TxnID")
            ...
            for entity in batch:
                failed.append({"entity": entity, "error": str(e)})
    ...
    def batch_create_parallel(
        ...
        logger.info(
            f"Processing {len(batches)} batches in parallel (max {self.max_workers} workers)..."
        )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    results["succeeded_ids"].extend(
                        [s.get("Id", "") for s in succeeded]
                    )
        ...
        logger.info(
            f"Batch processing complete: Succeeded={results['succeeded_count']}, Failed={len(results['failed'])}"
        )
    ...
    def batch_create_optimized(
                    ...
                    if future.done():
                        ...
                        try:
                            ...
                            if progress_callback:
                                ...
                                rate = (
                                    processed_entities / elapsed * 3600
                                    if elapsed > 0
                                    else 0
                                )
                                progress_callback(
                                    processed_entities, len(entities), rate
                                )
        ...
        actual_throughput = (
            len(entities) / duration_seconds * 3600 if duration_seconds > 0 else 0
        )
        ...
        logger.info(
            f"$25M BATCH COMPLETE: {results['succeeded_count']} succeeded, {len(results['failed'])} failed"
        )
        logger.info(
            f"  Duration: {duration_seconds:.1f}s | Throughput: {actual_throughput:.0f}/hour"
        )
    ...
    def query_count(self, entity_type: str, oauth_manager: Optional[Any] = None) -> int:
        ...
        try:
            ...
            try:
                results = self.query(
                    entity_type, max_results=1, oauth_manager=oauth_manager
                )
                ...
                logger.warning(
                    f"Query count fallback also failed for {entity_type}: {fallback_err}"
                )
            return 0
    ...
    def query(
        ...
        while True:
            ...
            try:
                response = self._make_request(
                    "GET", endpoint, oauth_manager=oauth_manager
                )
    ...
    def cleanup_migration(
        self, migration_id: str, oauth_manager: Optional[Any] = None
    ) -> Dict:
        """
        ...
        # HIGH FIX: Use logger instead of print
        logger.info(
            f"Rolling back migration {migration_id}... Found {len(entities)} entities to delete"
        )
        ...
        return {"deleted": deleted, "failed": failed, "total": len(entities)}
    ...
    def query_raw(self, query: str, oauth_manager: Optional[Any] = None) -> List[Dict]:
        ...
        try:
            ...
            response_data = self._make_request(
                method="GET", endpoint=endpoint, oauth_manager=oauth_manager
            )
    ...
    def batch_upload(
        ...
        logger.info(
            f"  Total entities: {sum(len(v) for v in entities_by_type.values())}"
        )
        ...
        for entity_type in sorted_types:
            ...
            try:
                ...
                for failed_item in failed_items[:20]:
                    error_msg = (
                        f"{entity_type}: {failed_item.get('error', 'Unknown error')}"
                    )
