...
class TestEntityDependencyOrdering:
    ...
    def test_orchestrator_entity_order(self):
        ...
        entity_order = [
            ...
            'Employees', 'Invoices', 'Bills', 'Payments',
        ]
...
class TestIdMappingPropagation:
    ...
    def test_invoice_uses_mapped_customer_id(self, transformer, qbd_customers, qbd_items, qbd_invoices):
    ...
    def test_payment_uses_mapped_customer_and_invoice_ids(
        ...
        assert payment['CustomerRef']['value'] == transformer.id_mapping['customers']['CUST-001']
    ...
    def test_bill_uses_mapped_vendor_id(self, transformer, qbd_accounts, qbd_vendors, qbd_bills):
        ...
        assert bill['VendorRef']['value'] == transformer.id_mapping['vendors']['VEND-001']
    ...
    def test_all_plural_mappings_exist(self):
        ...
        for plural, expected_singular in expected_mappings.items():
            ...
            orch._migrate_entity(
                mock_client, mock_transformer, plural,
                [{'Name': 'Test', 'ListID': 'X'}], {},
            )
            actual = mock_client.create_entity.call_args[0][0]
            assert actual == expected_singular, (
...
class TestTokenRefresh:
    ...
    def test_base_access_token_updated_on_refresh(self, mock_qbo_client, mock_oauth_manager):
...
class TestTrialBalanceValidation:
    ...
    def test_balanced_trial_balance(self, transformer):
        ...
        accounts = [
            {'Name': 'Cash', 'AccountType': 'Bank', 'Balance': '10000.00',
             'ListID': 'A1', 'IsActive': True},
            {'Name': 'Revenue', 'AccountType': 'Income', 'Balance': '10000.00',
             'ListID': 'A2', 'IsActive': True},
        ]
        for acc in accounts:
            transformer.transform_account(acc)
    ...
    def test_unbalanced_trial_balance_detected(self, transformer):
        ...
        accounts = [
            {'Name': 'Cash', 'AccountType': 'Bank', 'Balance': '5000.00',
             'ListID': 'A1', 'IsActive': True},
            {'Name': 'Revenue', 'AccountType': 'Income', 'Balance': '3000.00',
             'ListID': 'A2', 'IsActive': True},
        ]
        for acc in accounts:
            transformer.transform_account(acc)

        assert transformer.trial_balance['debits'] != transformer.trial_balance['credits']

    def test_transform_outputs_trial_balance_info(self, transformer):
        """The transform() result should include trial_balance dict."""
        data = {
            'Account': [
                {'Name': 'Bank', 'AccountType': 'Bank', 'Balance': '1000',
                 'ListID': 'A1', 'IsActive': True},
                {'Name': 'Equity', 'AccountType': 'Equity', 'Balance': '1000',
                 'ListID': 'A2', 'IsActive': True},
            ]
        }
    ...
    def test_negative_balance_handling(self, transformer):
        ...
        acc = {'Name': 'Overdrawn', 'AccountType': 'Bank', 'Balance': '-500.00',
               'ListID': 'A-NEG', 'IsActive': True}
        transformer.transform_account(acc)
