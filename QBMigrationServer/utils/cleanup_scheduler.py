...
class CleanupScheduler:
    ...
    def run_cleanup(self):
        ...
        try:
            with self.app.app_context():
                ...
                logger.info(f"Found {len(migrations_to_clean)} migrations needing cleanup")
    ...
    def _find_migrations_needing_cleanup(self):
        ...
        try:
            ...
            all_migrations = set(completed_not_cleaned + stuck_migrations + expired_migrations)
    ...
    def _cleanup_migration(self, migration):
        ...
        """
        try:
            logger.info(f"Cleaning up migration {migration.migration_id} (status: {migration.status})")
            ...
            if all(cleanup_results.values()):
                ...
                logger.warning(f"Partial cleanup for migration {migration.migration_id}: {cleanup_results}")
            
        except Exception as e:
            logger.exception(f"Failed to cleanup migration {migration.migration_id}: {str(e)}")
