...
def validate_pagination_param(value, param_name, default, min_val, max_val):
    ...
    if value_int < min_val or value_int > max_val:
        logger.warning(
            f"{param_name} out of range: {value_int} (allowed: {min_val}-{max_val})"
        )
        return default
...
def create_migration():
    ...
    try:
        ...
        company_name = (
            safe_file_name.rsplit(".", 1)[0] if safe_file_name else "Unknown Company"
        )
        ...
        logger.info(
            f"Created migration {migration_id} for user {user_id}, destination: {destination}"
        )

        return (
            jsonify(
                {"success": True, "migration_id": migration_id, "status": "pending"}
            ),
            201,
        )
    ...
    try:
        ...
        if status_filter:
            ...
            if not re.match(r"^[a-z_]+$", status_filter):
                logger.warning(
                    f"Status filter contains invalid characters: {status_filter[:50]}"
                )
                return (
                    jsonify(
                        {"success": False, "error": "Invalid status filter format"}
                    ),
                    400,
                )
        ...
        for migration in pagination.items:
            ...
            if hasattr(migration, "get_error_message") and callable(
        ...
        db.session.remove()
        return (
            jsonify({"success": False, "error": "Failed to retrieve migrations"}),
            500,
        )
...
def get_migration(migration_id):
    ...
    if not validate_migration_id(migration_id):
        logger.warning(
            f"Invalid migration ID format: {migration_id[:50] if migration_id else 'None'}"
        )
    ...
    try:
        ...
        db.session.rollback()
        db.session.remove()
        return jsonify({"success": False, "error": "Failed to retrieve migration"}), 500
...
def start_migration(migration_id):  # noqa: C901
    ...
    try:
        ...
        migration = (
            ...
            .filter_by(migration_id=migration_id, user_id=user_id)
            .with_for_update()
            .first()
        )
...
def cancel_migration(migration_id):
    ...
    try:
        ...
        if hasattr(migration, "mark_ec2_terminated") and cleanup_results.get(
            "instance_terminated"
        ):
        ...
        db.session.rollback()
        db.session.remove()
        return jsonify({"success": False, "error": "Failed to cancel migration"}), 500
...
def retry_migration(migration_id):
    ...
    try:
        ...
        if migration.status != "failed":
            return (
                jsonify(
                    {"success": False, "error": "Only failed migrations can be retried"}
                ),
                400,
            )
        ...
        if hasattr(migration, "increment_retry") and callable(
        ...
        max_retries = getattr(migration, "max_retries", 3)
        logger.info(
            f"Migration {migration_id} reset for retry (attempt {retry_count}/{max_retries})"
        )
        ...
        db.session.rollback()
        db.session.remove()
        return jsonify({"success": False, "error": "Failed to retry migration"}), 500
...
def delete_migration(migration_id):
    ...
    try:
        ...
        db.session.rollback()
        db.session.remove()
        return jsonify({"success": False, "error": "Failed to delete migration"}), 500
