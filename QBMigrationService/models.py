...
class Payment(QBEntity):
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        line_total = sum(
            Decimal(str(line.get("Amount", 0))) for line in self.applied_to_invoices
        )

        if abs(line_total - self.total_amt) > Decimal("0.01"):
            errors.append(
                f"Line totals ({line_total}) don't match total amount ({self.total_amt})"
            )
...
class JournalEntry(QBEntity):
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        debits = sum(line.amount for line in self.lines if line.posting_type == "Debit")
        credits = sum(
            line.amount for line in self.lines if line.posting_type == "Credit"
        )

        if abs(debits - credits) > Decimal("0.01"):
            errors.append(
                f"Journal entry not balanced: Debits={debits}, Credits={credits}"
            )
