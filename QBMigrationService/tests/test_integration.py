...
class TestFrontendBackendIntegration:
    ...
    def test_live_status_polling_response_format(self, backend_url):
        ...
        expected_fields = [
            'phase', 'phase_number', 'percentage', 'phases',
            'current_entity', 'status_message', 'elapsed_seconds'
        ]
...
class TestQBDConnection:
    ...
    def qbd_reader_path(self):
        ...
        return os.path.join(
            ...
            '..', '..', 'QBDesktopReader', 'bin', 'Release', 'QBDesktopReader.exe'
        )
    ...
    def test_qbd_reader_executable_exists(self, qbd_reader_path):
        ...
        possible_paths = [
            qbd_reader_path,
            os.path.join(os.path.dirname(__file__), '..', '..', 'QBDesktopReader', 'bin', 'Debug', 'QBDesktopReader.exe'),
            os.path.join(os.path.dirname(__file__), '..', '..', 'QBDesktopReader', 'bin', 'Release', 'net8.0-windows', 'QBDesktopReader.exe')
        ]
...
class TestMigrationFlow:
    ...
    def test_migration_phase_transitions(self):
        ...
        for start, end, expected_phase in phase_ranges:
            for percent in range(start, end):
                ...
                assert actual == expected_phase, f"At {percent}%, expected {expected_phase} but got {actual}"
    ...
    def test_audit_certificate_contains_required_fields(self):
        ...
        required_fields = [
            "migration_id", "company_name", "source_hash", "destination_hash",
            "trial_balance_verified", "forensic_status"
        ]
