...
def create_app(config_name=None):  # noqa: C901
    ...
    if not backup_key:
        ...
        if config_name == "production":
            logger.warning(
                "BACKUP_ENCRYPTION_KEY not set — generated a temporary key. "
                "Set BACKUP_ENCRYPTION_KEY in your .env for stable token encryption across restarts. "
    ...
    if config_name == "production":
        ...
        if ami_id:
            ...
            if not ami_id.startswith("ami-") or len(ami_id) < 12:
                logger.warning(
                    "Invalid AMI ID format: '%s'. "
                    "AMI IDs must start with 'ami-' and be at least 12 characters.",
                    ami_id,
                )
            else:
                logger.info(
                    "AWS configuration: Region=%s, AMI=%s...", aws_region, ami_id[:12]
                )
        else:
            logger.warning(
                "AWS_EC2_AMI_ID not set — EC2 worker provisioning will not work."
            )
    else:
        logger.info("AWS validation skipped for %s environment", config_name)
    ...
    # Override with ALLOW_LOCALHOST_CORS=true for Docker-local production testing.
    if is_production() and "localhost" in str(allowed_origins):
        if os.getenv("ALLOW_LOCALHOST_CORS", "").lower() not in ("true", "1"):
            raise ValueError(
                ...
                "If running Docker locally, set ALLOW_LOCALHOST_CORS=true to override. "
                f"Current origins: {allowed_origins}"
            )
        else:
            app.logger.warning(
                "SECURITY: localhost CORS origins allowed via ALLOW_LOCALHOST_CORS override. "
                "Ensure this is DISABLED in real production deployments."
            )
