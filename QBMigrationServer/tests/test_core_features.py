...
class TestReportGeneration:
    ...
    def test_generate_variance_report(
        self, client, auth_headers, test_migration, db_session
    ):
        ...
        response = client.post(
            ...
            json={"type": "variance", "migration_id": test_migration.migration_id},
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_generate_health_report(
        ...
        response = client.post(
            "/api/reports/generate",
            headers=auth_headers,
            json={"type": "health", "migration_id": test_migration.migration_id},
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_generate_discrepancy_report(
        ...
        response = client.post(
            "/api/reports/generate",
            headers=auth_headers,
            json={"type": "discrepancy", "migration_id": test_migration.migration_id},
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_download_report(self, client, auth_headers, test_migration, db_session):
        ...
        response = client.get(
            f"/api/reports/{report_id}/download", headers=auth_headers
        )
...
class TestReconciliationShield:
    ...
    def test_get_trial_balance_verified(
        self, client, auth_headers, test_migration, db_session
    ):
    ...
    def test_get_trial_balance_discrepancy(
        self, client, auth_headers, test_migration, db_session
    ):
...
class TestCasewareBundle:
    ...
    def test_export_caseware_creates_bundle(
        self, client, auth_headers, test_migration, db_session
    ):
    ...
    def test_download_caseware_bundle(
        self, client, auth_headers, test_migration, db_session
    ):
    ...
    def test_caseware_bundle_without_generation_fails(
        self, client, auth_headers, test_migration
    ):
        ...
        data = response.get_json()
        assert (
            "not yet generated" in data["error"].lower()
...
class TestDiscrepancyDoctor:
    ...
    def test_get_discrepancies_with_data(
        self, client, auth_headers, test_migration, db_session
    ):
...
class TestAuditCertificate:
    ...
    def test_certificate_not_available_for_incomplete(
        self, client, auth_headers, test_migration
    ):
...
class TestErrorHandling:
    ...
    def test_invalid_report_type(
        ...
        response = client.post(
            "/api/reports/generate",
            headers=auth_headers,
            json={"type": "invalid_type", "migration_id": test_migration.migration_id},
        )
        assert response.status_code == 400
