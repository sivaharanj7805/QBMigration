...
class License(db.Model):
    ...
    @classmethod
    def create_license(
        cls, tier="starter", user_email=None, order_id=None, expires_in_days=None
    ):
        """
        ...
        if tier not in LICENSE_TIERS:
            raise ValueError(
                f"Invalid tier: {tier}. Must be one of: {list(LICENSE_TIERS.keys())}"
            )
    ...
    def activate(self, hardware_fingerprint, user_id=None):
        ...
        activation = LicenseActivation(
            hardware_fingerprint=hardware_fingerprint, action="activate", success=True
        )
...
class LicenseActivation(db.Model):
    ...
    license_id = db.Column(
        db.Integer, db.ForeignKey("licenses.id", ondelete="CASCADE"), nullable=False
    )

    # Action Details
    action = db.Column(
        db.String(20), nullable=False
    )  # activate, deactivate, validate, revoke
    hardware_fingerprint = db.Column(db.String(128))
