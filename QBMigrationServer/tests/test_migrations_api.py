...
class TestListMigrations:
    ...
    def test_list_migrations_with_data(
    ...
    def test_list_migrations_pagination(
        self, authenticated_client, db_session, test_user
    ):
    ...
    def test_list_migrations_status_filter(
        self, authenticated_client, db_session, test_user
    ):
    ...
    def test_list_migrations_invalid_status_filter(
    ...
    def test_list_migrations_does_not_show_other_users(
        self, authenticated_client, db_session, test_user
    ):
    ...
    def test_list_migrations_ordered_by_newest_first(
        self, authenticated_client, db_session, test_user
    ):
...
class TestGetMigrationStatus:
    ...
    def test_get_status_with_current_step(
        self, authenticated_client, db_session, test_migration
    ):
    ...
    def test_get_status_requires_auth(self, client, db_session, test_migration):
        """Unauthenticated request returns 401."""
        response = client.get(f"/api/migrations/{test_migration.migration_id}/status")
        assert response.status_code == 401
...
class TestCancelMigration:
    ...
    @pytest.mark.parametrize(
        "initial_status", ["pending", "uploaded", "provisioning", "processing"]
    )
    ...
    def test_cancel_disallowed_states(
        self, initial_status, authenticated_client, db_session, test_migration
    ):
    ...
    def test_cancel_requires_auth(self, client, db_session, test_migration):
        """Unauthenticated request returns 401."""
        response = client.post(f"/api/migrations/{test_migration.migration_id}/cancel")
        assert response.status_code == 401
...
class TestRetryMigration:
    ...
    def test_retry_non_failed_migration(
        self, authenticated_client, db_session, test_migration
    ):
    ...
    def test_retry_requires_auth(self, client, db_session, test_migration):
        """Unauthenticated request returns 401."""
        response = client.post(f"/api/migrations/{test_migration.migration_id}/retry")
        assert response.status_code == 401

    def test_retry_increments_count_each_time(
        self, authenticated_client, db_session, test_migration
    ):
...
class TestDeleteMigration:
    ...
    def test_delete_success(
        self, mock_aws_cls, authenticated_client, db_session, test_migration
    ):
        ...
        response = authenticated_client.delete(f"/api/migrations/{migration_id}")
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    @patch("api.migrations.AWSMigrationManager")
    def test_delete_completed_migration(
        self, mock_aws_cls, authenticated_client, db_session, test_migration
    ):
    ...
    def test_delete_requires_auth(self, client, db_session, test_migration):
        """Unauthenticated request returns 401."""
        response = client.delete(f"/api/migrations/{test_migration.migration_id}")
        assert response.status_code == 401
...
class TestMigrationStats:
    ...
    def test_stats_success_rate_with_failures(
        self, authenticated_client, db_session, test_user
    ):
...
class TestInvalidMigrationId:
    ...
    def test_sql_injection_in_status(self, authenticated_client, db_session):
        ...
        response = authenticated_client.get("/api/migrations/' OR 1=1 --/status")
        assert response.status_code == 400
