...
def lambda_handler(event, context):
    ...
    return {"statusCode": 200, "body": json.dumps("Processing triggered")}
...
def trigger_migration_processing(session_id: str, bucket: str, key: str):
    ...
    if api_url:
        ...
        if not api_key:
            raise ValueError(
                "INTERNAL_API_KEY not configured - cannot call internal API securely"
            )

        req_data = json.dumps(
            {"session_id": session_id, "s3_bucket": bucket, "s3_key": key}
        ).encode("utf-8")
